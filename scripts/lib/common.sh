...
set_defaults() {
    ...
    DB_PASSWORD_SECRET="${DB_PASSWORD_SECRET:-db-credentials}"
    DB_PASSWORD_KEY="${DB_PASSWORD_KEY:-password}"
}
...
PIPELINE_VARS='${PIPELINE_NAMESPACE} ${GIT_REPO_URL} ${GIT_REPO_DEFAULT_BRANCH} ${IMAGE_NAME} ${IMAGE_TAG} ${REGISTRY_CLUSTER_HOST} ${REGISTRY_CLUSTER_PORT} ${DEPLOYMENT_NAMESPACE} ${DEPLOYMENT_NAME} ${DB_URL} ${DB_USERNAME} ${DB_PASSWORD_SECRET} ${DB_PASSWORD_KEY}'
PIPELINE_RUN_VARS='${PIPELINE_NAMESPACE} ${GIT_REPO_URL} ${GIT_REPO_DEFAULT_BRANCH} ${IMAGE_NAME} ${IMAGE_TAG} ${REGISTRY_CLUSTER_HOST} ${REGISTRY_CLUSTER_PORT} ${DEPLOYMENT_NAMESPACE} ${DEPLOYMENT_NAME} ${PIPELINE_MODE} ${DB_URL} ${DB_USERNAME} ${DB_PASSWORD_SECRET} ${DB_PASSWORD_KEY}'
